...
# Data Parameters
#tf.flags.DEFINE_string("input_text_file", "./data/neg_htt.txt", "Test text data source to evaluate.")
tf.flags.DEFINE_string("input_text_file", "./data/content3zw.txt", "Test text data source to evaluate.")
#tf.flags.DEFINE_string("input_text_file", "./data/pos_question.txt", "Test text data source to evaluate.")
tf.flags.DEFINE_string("input_label_file", "", "Label file for test text data source.")

# Eval Parameters
tf.flags.DEFINE_integer("batch_size", 100, "Batch Size (default: 64)")
#tf.flags.DEFINE_string("checkpoint_dir", "C:\\Zorcius\\filtering\\cnn-tf-master-jq\\runs\\1509433942\\checkpoints", "Checkpoint directory from training run")
#tf.flags.DEFINE_string("checkpoint_dir", "C:\\Zorcius\\filtering\\cnn-tf-master-jq\\runs\\1509607446\\checkpoints", "Checkpoint directory from training run")
#tf.flags.DEFINE_string("checkpoint_dir", "./runs/1510195882/checkpoints", "Checkpoint directory from training run")
tf.flags.DEFINE_string("checkpoint_dir", "./runs/1510202744/checkpoints", "Checkpoint directory from training run")
...
sentences = data_helpers.padding_sentences(x_raw, '<PADDING>', padding_sentence_length = max_document_length)
...
with open(out_path,'w') as fw:
    ...
    #text=list(open(FLAGS.input_text_file, encoding='gbk').readlines())
    #text=list(open(FLAGS.input_text_file).readlines())
    text=[line.strip('\n') for line in text]
